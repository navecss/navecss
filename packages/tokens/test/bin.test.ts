...
function runNode(
  ...
  try {
    const stdout = execFileSync(process.execPath, [...nodeFlags, scriptPath, ...args], {
      cwd,
      encoding: 'utf8',
    })
    return { status: 0, stdout, stderr: '' }
  } catch (error) {
    const failure = error as { status: number | null; stderr: string; stdout: string }
...
describe('AC-token-build-03 covers: R3 (bin-level: no other flag or option exists)', () => {
...
describe('AC-token-build-04 covers: R4 (real, out-of-process exit codes; the SET is exactly {0, 1, 2})', () => {
...
describe('AC-token-build-04 covers: R4 (--overrides shape, malformed --source, --help)', () => {
...
describe('AC-token-build-35 covers: R35 (no test asserted this before now)', () => {
...
describe('AC-token-build-08 covers: R9 (out-of-process black-box clause)', () => {
...
describe('AC-token-build-14 covers: R14 (installedCoreVersion, real resolution, both failure directions)', () => {
...
describe('AC-token-build-17 covers: R17 (property 3, second dated precision — provenance on the FAILURE line)', () => {
  /**
...
describe('AC-token-build-15 covers: R15 (dated precision — a --source whose JSON ROOT is not an object)', () => {
  /**
...
describe('R3-03 covers: R15/R21 (the file label is delimited at every door)', () => {
  it.each([
